	...
	<input type="hidden" name="id" value="${outport.id }">  
		<div style="margin: 10px 20px">
			...
			<div class="layui-form-item">
				...
				<div class="layui-input-inline">
				<input type="text" name="number" placeholder="数量"  value="${outport.number }"  autocomplete="off"
						class="layui-input">
				</div>
				<label class="layui-form-label">退货价格</label>
				<div class="layui-input-inline">
					<input type="text" name="outportprice" placeholder="价格"  value="${outport.outportprice }" autocomplete="off"
						class="layui-input">
				</div>
			</div>
			<div class="layui-form-item">
				...
				<div class="layui-input-block">
					<input type="radio" name="paytype" title="支付宝" value="支付宝" ${outport.paytype eq '支付宝'?'checked':'' }>
					<input type="radio" name="paytype" title="微信" value="微信"  ${outport.paytype eq '微信'?'checked':'' }>
					<input type="radio" name="paytype" title="银联" value="银联"  ${outport.paytype eq '银联'?'checked':'' }>
				</div>
			</div>
			<div class="layui-form-item">
				<div class="layui-form-item">
					...
					<div class="layui-input-block">
						<textarea name="remark" placeholder="请输入"
							class="layui-textarea">${outport.remark }</textarea>
					</div>
				</div>
			</div>
		</div>
...
var form;
		layui.use([ 'form', 'layer', 'jquery','upload' ], function() {
			...
			form.on("submit(formSubmit)", function(data) {
				...
				$.post("${ctx}/outport/updateOutport.action?" + params, function(
						result) {
