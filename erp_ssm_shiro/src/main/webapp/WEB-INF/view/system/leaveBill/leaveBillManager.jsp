...
<script type="text/html" id="leaveBillListBar">
  {{#  if(d.state ==0 ){ }}
    ...
    <a class="layui-btn layui-btn-xs layui-btn-normal" lay-event="submitApply">提交申请</a>
  {{#  } else if(d.state ==1) { }}
     <a class="layui-btn layui-btn-xs layui-btn-warm" lay-event="viewApprovals">审批查询</a>
  {{#  }else if(d.state ==2) { }}
	 <a class="layui-btn layui-btn-xs layui-btn-warm" lay-event="viewApprovals">审批查询</a>
  {{#  }else if(d.state ==3) { }}
	  <a class="layui-btn layui-btn-xs" lay-event="edit">编辑</a>
...
<script type="text/javascript">
    ...
    layui.use(['form','layer','table','laydate'],function(){
        ...
        tableIns = table.render({
            elem: '#leaveBillList',
            url : '${ctx }/leaveBill/loadAllLeaveBill.action',
            cellMinWidth : 95,
            ...
            cols : [[
                ...
                {title: '操作',templet:'#leaveBillListBar',fixed:"right",width:200,align:"center"}
            ]]
        });
        ...
        $(".search_btn").on("click",function(){
            ...
            table.reload('leaveBillListTable', {
                url: '${ctx }/leaveBill/loadAllLeaveBill.action?'+params
            });
        });
        //监听头工具栏事件
        ...
        table.on('tool(leaveBillList)', function(obj){
            ...
            if(layEvent === 'edit'){ //编辑
                ...
                layer.confirm('确定删【'+data.title+'】请假单吗？',{icon:3, title:'提示信息'},function(index){
                    $.post("${ctx}/leaveBill/delLeaveBill.action",{
            ...
            }else if(layEvent==="viewApprovals"){
            	   var index = layui.layer.open({
                       title : "审批查询",
                       type : 2,
                       area:["800px","500px"],
                       content : "${ctx }/workFlow/toViewApprovals.action?id="+data.id,
                       success : function(layero, index){
                           setTimeout(function(){
                               layui.layer.tips('点击此处返回请假单列表', '.layui-layer-setwin .layui-layer-close', {
                                   tips: 3
