...
<body class="childrenBody">
	<form class="layui-form" method="post" id="frm">
	  ...
	  <div class="layui-form-item" style="text-align: center;">
	      <c:forEach var="btn" items="${outcomes }" varStatus="sn">
	       <c:choose>
		  			<c:when test="${sn.index==0 }">
		      			<input class="layui-btn btn" type="button" value="${btn }" />
		  			</c:when>
		  			<c:otherwise>
		  				<input class="layui-btn layui-btn-warm btn" type="button" value="${btn }" />
		  			</c:otherwise>
	       </c:choose>
	      </c:forEach>
  	  </div>
	...
	layui.use(['form','jquery','layer','table'],function(){
	    ...
	    //批注信息表
	      var  tableIns = table.render({
	        elem: '#commentList',
	        url : '${ctx }/workFlow/loadCommentsByTaskId.action?taskId=${workFlowVo.taskId}',
	        cellMinWidth : 95,
	        toolbar:false,
	        page : true,
	        height : "full-120",
	    ...
	    $(".btn").on('click',function(){
	       ...
	       var id='${leaveBill.id}';
	       $.post("${ctx}/workFlow/doTask.action",{outcome:outcome,taskId:taskId,comment:comment,id:id},function(obj){
	    	  layer.msg(obj.msg);
