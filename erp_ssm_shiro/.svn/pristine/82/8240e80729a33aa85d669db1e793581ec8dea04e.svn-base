...
layui.use(['form','layer','laydate','table','laytpl'],function(){
    ...
    //登入权限列表
        ...
        cols : [[
            ...
            {field: 'name', title: '权限名称', width:180, align:"center"},
            {field: 'percode', title: '权限编码', width:180, align:"center"},
    ...
    function deletePermission(id){
    	 layer.confirm('确定删除选中的权限？', {icon: 3, title: '提示信息'}, function (index) {
   		  $.post("${ctx}/permission/delPermission.action?id="+id,function(result){
    ...
    $(".batchDel_btn").on("click",function(){
         ...
         url="${ctx}/permission/delPermissions.action?1=1";
     if(data.length > 0) {
         for (var i in data) {
             url+="&ids="+data[i].id;
         }
